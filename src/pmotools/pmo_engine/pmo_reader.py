    ...
    def combine_multiple_pmos(pmos : list[dict]):
                ...
                if target["target_name"] in target_info_out_index_key:
                    ...
                    if len(pmo_out["targeted_genomes"]) > 1:
                        if "insert_location" in target_copy:
                            # update genome_id
                            target_copy["insert_location"]["genome_id"] = targeted_genomes_old_index_key[pmo_index][target_copy["insert_location"]["genome_id"]]
                        if "location" in target_copy["forward_primer"]:
                            # update genome_id
                            target_copy["forward_primer"]["location"]["genome_id"] = targeted_genomes_old_index_key[pmo_index][target_copy["forward_primer"]["location"]["genome_id"]]
                        if "location" in target_copy["reverse_primer"]:
                            # update genome_id
                            target_copy["reverse_primer"]["location"]["genome_id"] = targeted_genomes_old_index_key[pmo_index][target_copy["reverse_primer"]["location"]["genome_id"]]
        ...
        for pmo_index, pmo in enumerate(pmos[1:], start=1):
            for panel_index, panel in enumerate(pmo["panel_info"]):
                ...
                if panel_copy["panel_name"] in panel_info_out_index_key:
                    panel_info_old_index_key[pmo_index][panel_index] = panel_info_out_index_key[panel_copy["panel_name"]]
                else:
        ...
        for pmo_index, pmo in enumerate(pmos[1:], start=1):
            for specimen_info_index, specimen_info in enumerate(pmo["specimen_info"]):
                ...
                specimen_info_copy["project_id"] = project_info_old_index_key[pmo_index][specimen_info_copy["project_id"]]
        ...
        for pmo_index, pmo in enumerate(pmos[1:], start=1):
            for library_sample_info_index, library_sample_info in enumerate(pmo["library_sample_info"]):
                # checkin for duplicates
                if library_sample_info["library_sample_name"] in library_sample_names:
                    duplicate_library_sample_names.append(library_sample_info["library_sample_name"])
                ...
                library_sample_info_copy["specimen_id"] = specimen_info_old_index_key[pmo_index][library_sample_info_copy["specimen_id"]]
                library_sample_info_copy["panel_id"] = panel_info_old_index_key[pmo_index][library_sample_info_copy["panel_id"]]
                library_sample_info_copy["sequencing_info_id"] = sequencing_info_old_index_key[pmo_index][library_sample_info_copy["sequencing_info_id"]]
                ...
                library_sample_info_old_index_key[pmo_index][library_sample_info_index] = new_index
        ...
        pmo_out["bioinformatics_methods_info"] = copy.deepcopy(pmos[0]["bioinformatics_methods_info"])
        #key1 pmo_index, key2 old_index, val new_index
        ...
        for pmo_index, pmo in enumerate(pmos[1:], start=1):
            for bioinformatics_methods_info_index, bioinformatics_methods_info in enumerate(pmo["bioinformatics_methods_info"]):
                ...
                pmo_out["bioinformatics_methods_info"].append(copy.deepcopy(bioinformatics_methods_info))
        ...
        pmo_out["bioinformatics_run_info"] = copy.deepcopy(pmos[0]["bioinformatics_run_info"])
        #key1 pmo_index, key2 old_index, val new_index
        ...
        for pmo_index, pmo in enumerate(pmos[1:], start=1):
            for bioinformatics_run_info_index, bioinformatics_run_info in enumerate(pmo["bioinformatics_run_info"]):
                ...
                bioinformatics_run_info_copy["bioinformatics_methods_id"] = bioinformatics_methods_info_old_index_key[pmo_index][bioinformatics_run_info_index]
                ...
                if pmo["target_info"][representative_microhaplotypes["target_id"]]["target_name"] in representative_microhaplotypes_out_index_key:
                    ...
                    for adding_microhap_index, adding_microhap in enumerate(representative_microhaplotypes["microhaplotypes"]):
                        ...
                        if not found:
                            ...
                            pmo_out["representative_microhaplotypes"]["targets"][representative_microhaplotypes_out_index_key[pmo["target_info"][representative_microhaplotypes["target_id"]]["target_name"]]]["microhaplotypes"].append(copy.deepcopy(adding_microhap))
                    ...
                    pmo_out["representative_microhaplotypes"]["targets"].append(copy.deepcopy(representative_microhaplotypes))
        ...
        pmo_out["detected_microhaplotypes"] = copy.deepcopy(pmos[0]["detected_microhaplotypes"])
        for pmo_index, pmo in enumerate(pmos[1:], start=1):
            # update indexes
            for detected_microhaplotypes in pmo["detected_microhaplotypes"]:
                ...
                for library_sample in detected_microhaplotypes_copy["library_samples"]:
                    ...
                    library_sample["library_sample_id"] = library_sample_info_old_index_key[pmo_index][library_sample["library_sample_id"]]
                detected_microhaplotypes_copy["bioinformatics_run_id"] = bioinformatics_run_info_old_index_key[pmo_index][detected_microhaplotypes_copy["bioinformatics_run_id"]]
                # append after the indexes have been updated
                pmo_out["detected_microhaplotypes"].append(detected_microhaplotypes_copy)
        ...
        for pmo_index in pmo_indexes_with_read_counts_by_stage:
            ...
            if 0 == pmo_index:
                pmo_out["read_counts_by_stage"] = copy.deepcopy(pmos[pmo_index]["read_counts_by_stage"])
            else:
                ...
                for read_counts_by_stage in pmos[pmo_index]["read_counts_by_stage"]:
                    ...
                    for read_counts_by_library_sample_by_stage in read_counts_by_stage_copy["read_counts_by_library_sample_by_stage"]:
                        ...
                        read_counts_by_library_sample_by_stage["library_sample_id"] = library_sample_info_old_index_key[pmo_index][read_counts_by_library_sample_by_stage["library_sample_id"]]
                    read_counts_by_stage_copy["bioinformatics_run_id"] = bioinformatics_run_info_old_index_key[pmo_index][read_counts_by_stage_copy["bioinformatics_run_id"]]
