...
def microhaplotype_table_to_pmo_dict(
    ...
    representative_microhaplotype_dict = create_representative_microhaplotype_dict(microhaplotype_table, target_name_col, seq_col, genome_id, chrom_col, start_col, end_col, ref_seq_col, strand_col, alt_annotations_col,
    ...
    output_data = {"representative_microhaplotypes": representative_microhaplotype_dict,
...
def create_representative_microhaplotype_dict(
    ...
    optional_cols = [chrom_col, start_col, end_col, ref_seq_col, strand_col,
                     alt_annotations_col, microhaplotype_name_col, pseudocigar_col, quality_col]
    masking_cols = [masking_seq_start_col,
                    masking_seq_segment_size_col, masking_replacement_size_col]
    ...
    unique_table = microhaplotype_table[all_cols].drop_duplicates(
...
def create_detected_microhaplotype_dict(
    ...
    if additional_mhap_detected_cols:
        check_additional_columns_exist(
            df, additional_mhap_detected_cols)
        mhap_cols += additional_mhap_detected_cols

    # Find IDs for targets and mhaps in the representative table
    df = get_target_id_in_representative_mhaps(
        df, representative_microhaplotype_dict)
    df = get_mhap_index_in_representative_mhaps(
        df, representative_microhaplotype_dict)

    # Build detected mhap table
    mhap_detected = build_detected_mhap_dict(
        df, bioinformatics_run_name, mhap_cols)
...
def get_target_id_in_representative_mhaps(df, representative_dict):
    target_name_to_mhaps_target_id = {entry['target_name']: i for i, entry in enumerate(
        representative_dict['targets'])}
