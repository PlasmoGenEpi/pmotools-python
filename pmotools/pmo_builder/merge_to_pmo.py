...
def merge_to_pmo(
    ...
    _replace_names_with_IDs(specimen_info, project_info, library_sample_info,
                            sequencing_info, panel_info, mhap_info, bioinfo_run_info)
...
def _report_missing_IDs(missing_projects, missing_sequencing, missing_specimen, missing_panels, missing_targets, missing_bioinfo_runs, missing_libs):
    if any([missing_projects, missing_sequencing, missing_specimen, missing_panels, missing_targets, missing_bioinfo_runs, missing_libs]):
        error_message = 'The following fields were found in one table and not another:\n'
        if missing_projects:
...
def _replace_names_with_IDs(specimen_info, project_info, library_sample_info, sequencing_info, panel_info, mhap_info, bioinfo_run_info):
    # SPECIMEN INFO
    ...
    missing_specimen = _replace_key_with_id(library_sample_info, specimen_info,
    ...
    lib_sample_lookup = _make_lookup(
        library_sample_info, 'library_sample_name')
    missing_libs = []
    for detected in mhap_info["detected_microhaplotypes"]:
        missing_libs += (_replace_key_with_id(detected["library_samples"],
                                              library_sample_info, 'library_sample_name', 'library_sample_id', lookup=lib_sample_lookup))

    # If any names were missing from reference tables error
    _report_missing_IDs(missing_projects, missing_sequencing, missing_specimen,
                        missing_panels, missing_targets, missing_bioinfo_runs, missing_libs)
