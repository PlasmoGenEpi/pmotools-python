...
def panel_info_table_to_json(
    ...
    missing_projects = replace_key_with_id(specimen_info, project_info,
                                           "project_name", "project_id")

    # EXPERIMENT INFO
    # replace with sequencing_info_id, specimen_id, panel_id
    missing_sequencing = replace_key_with_id(library_info, sequencing_info,
                                             'sequencing_info_name', "sequencing_info_id")
    missing_specimen = replace_key_with_id(library_info, specimen_info,
                                           'specimen_name', "specimen_id")
    missing_panels = replace_key_with_id(
        library_info, panel_info["panel_info"], 'panel_name', "panel_id")

    # REP MHAPS
    # replace target_name with ID
    missing_targets = replace_key_with_id(
    ...
    missing_bioinfo_runs = replace_key_with_id(
    ...
    for detected in mhap_info["detected_microhaplotypes"]:
        missing_libs += (replace_key_with_id(detected["library_samples"],
    ...
    report_missing_IDs(missing_projects, missing_sequencing, missing_specimen,
...
def report_missing_IDs(missing_projects, missing_sequencing, missing_specimen, missing_panels, missing_targets, missing_bioinfo_runs, missing_libs):
