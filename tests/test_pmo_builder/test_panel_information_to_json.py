...
class TestPanelInformationToJson(unittest.TestCase):
    ...
    def setUp(self):
        self.min_target_table = pd.DataFrame({'target_name': ['target1', 'target2', 'target3'], 'fwd_primer': [
            'CTT', 'CTA', 'TTG'], 'rev_primer': ['GTT', 'TGG', 'ATT']})
        self.min_target_json = [{'target_name': 'target1', 'forward_primer': {'seq': 'CTT'}, 'reverse_primer': {'seq': 'GTT'}},
                                {'target_name': 'target2', 'forward_primer': {
                                    'seq': 'CTA'}, 'reverse_primer': {'seq': 'TGG'}},
                                {'target_name': 'target3', 'forward_primer': {'seq': 'TTG'}, 'reverse_primer': {'seq': 'ATT'}}]
        self.genome_info = {
            'name': '3D7', 'url': 'somthing.com', 'genome_version': 1, 'taxon_id': 1}
        self.min_builder = PMOPanelBuilder(
            self.min_target_table, 'test_panel', self.genome_info, 'target_name', 'fwd_primer', 'rev_primer')
        ...
        self.builder_with_locations = PMOPanelBuilder(min_target_table_with_locations, 'test_panel', self.genome_info, 'target_name', 'fwd_primer', 'rev_primer',  forward_primers_start_col='target_start',
        ...
        min_target_table_with_locations_missing['insert_start'] = [
            2, None, None]
        min_target_table_with_locations_missing['insert_end'] = [3, 3, 3]
        min_target_table_with_locations_missing['chrom'] = 'chrom1'

        self.builder_with_missing_locations = PMOPanelBuilder(min_target_table_with_locations_missing, 'test_panel', self.genome_info, 'target_name', 'fwd_primer', 'rev_primer',  forward_primers_start_col='target_start',
                                                              forward_primers_end_col='insert_start',
                                                              reverse_primers_start_col='insert_end',
                                                              reverse_primers_end_col='target_end',
                                                              insert_start_col='insert_start',
    ...
    def test_check_targets_are_unique_fails(self):
        new_row = pd.DataFrame({'target_name': ['target1'], 'fwd_primer': [
            'TTT'], 'rev_primer': ['GGG']})
        duplicate_target_table = pd.concat(
            [self.min_target_table, new_row])
        builder = PMOPanelBuilder(
            duplicate_target_table, 'test_panel', self.genome_info, 'target_name', 'fwd_primer', 'rev_primer')
    ...
    def test_check_unique_target_info_fails(self):
        new_row = pd.DataFrame({'target_name': ['target4'], 'fwd_primer': [
            'CTT'], 'rev_primer': ['GTT']})
        duplicate_target_primers_table = pd.concat(
            [self.min_target_table, new_row])
        builder = PMOPanelBuilder(
            duplicate_target_primers_table, 'test_panel', self.genome_info, 'target_name', 'fwd_primer', 'rev_primer')
    ...
    def test_summarise_targets_missing_optional_info_nothing_missing(self):
        missing_insert_loc, missing_fwd_primer_loc, missing_rev_primer_loc = self.builder_with_locations.summarise_targets_missing_optional_info()
    ...
    def test_summarise_targets_missing_optional_info_with_missing(self):
        with self.assertWarns(UserWarning):
            missing_insert_loc, missing_fwd_primer_loc, missing_rev_primer_loc = self.builder_with_missing_locations.summarise_targets_missing_optional_info()
    ...
    def test_check_genome_info_fails_with_string(self):
        ...
        self.assertEqual(str(context.exception),
                         "genome_info must be a dict, but got str")
    ...
    def test_build_panel_info(self):
        ...
        expected_panel_info = {
            'panel_name': 'test_panel', "reactions": [{'reaction_name': "1", 'panel_targets': [0, 1, 2]}]}
    ...
    def test_build_panel_info_multi_reaction(self):
        ...
        target_table_with_reactions["reaction"] = [
            'reaction1', 'reaction2', 'reaction2']
        builder = PMOPanelBuilder(
            target_table_with_reactions, 'test_panel', self.genome_info, 'target_name', 'fwd_primer', 'rev_primer', reaction_name_col='reaction')
    ...
    def test_create_targets_dict_full_info(self, mock_summarise_targets_missing_optional_info, mock_check_unique_target_info, mock_check_targets_are_unique):
        ...
        expected_json = [
            {'target_name': 'target1', 'insert_location': {'genome_id': 0, 'chrom': 'chrom1', 'start': 2, 'end': 3}, 'forward_primer': {'seq': 'CTT', 'location': {
                'genome_id': 0, 'chrom': 'chrom1', 'end': 1, 'start': 2}}, 'reverse_primer': {'seq': 'GTT', 'location': {'genome_id': 0, 'chrom': 'chrom1', 'start': 3, 'end': 5}}},
            {'target_name': 'target2', 'insert_location': {'genome_id': 0, 'chrom': 'chrom1', 'start': 2, 'end': 3}, 'forward_primer': {'seq': 'CTA', 'location': {
                'genome_id': 0, 'chrom': 'chrom1', 'end': 1, 'start': 2}}, 'reverse_primer': {'seq': 'TGG', 'location': {'genome_id': 0, 'chrom': 'chrom1', 'start': 3, 'end': 5}}},
            {'target_name': 'target3', 'insert_location': {'genome_id': 0, 'chrom': 'chrom1', 'start': 2, 'end': 3}, 'forward_primer': {'seq': 'TTG', 'location': {
    ...
    def test_create_targets_dict_missing_info(self, mock_summarise_targets_missing_optional_info, mock_check_unique_target_info, mock_check_targets_are_unique):
        ...
        mock_summarise_targets_missing_optional_info.return_value = missing_insert_loc, missing_fwd_primer_loc, missing_rev_primer_loc
        target_info = self.builder_with_locations.create_targets_dict()
        expected_json = [
            {'target_name': 'target1', 'insert_location': {'genome_id': 0, 'chrom': 'chrom1', 'start': 2, 'end': 3}, 'forward_primer': {'seq': 'CTT', 'location': {
                'genome_id': 0, 'chrom': 'chrom1', 'end': 1, 'start': 2}}, 'reverse_primer': {'seq': 'GTT'}},
            {'target_name': 'target2', 'forward_primer': {'seq': 'CTA'}, 'reverse_primer': {
                'seq': 'TGG'}},
            {'target_name': 'target3', 'forward_primer': {'seq': 'TTG'}, 'reverse_primer': {'seq': 'ATT'}}]
        self.assertEqual(expected_json, target_info)
    ...
    def test_create_targets_dict_additional_info(self, mock_summarise_targets_missing_optional_info, mock_check_unique_target_info, mock_check_targets_are_unique):
        ...
        builder = PMOPanelBuilder(
            target_table, 'test_panel', self.genome_info, additional_target_info_cols=['extra_col', 'special_col'])
        mock_summarise_targets_missing_optional_info.return_value = [], [], []
        target_info = builder.create_targets_dict()
        expected_json = [{'target_name': 'target1', 'forward_primer': {'seq': 'CTT'}, 'reverse_primer': {'seq': 'GTT'}, 'extra_col': 'my target description', 'special_col': 'special target'},
                         {'target_name': 'target2', 'forward_primer': {
                             'seq': 'CTA'}, 'reverse_primer': {'seq': 'TGG'}, 'extra_col': 'my target description', 'special_col': 'special target'},
                         {'target_name': 'target3', 'forward_primer': {'seq': 'TTG'}, 'reverse_primer': {'seq': 'ATT'}, 'extra_col': 'my target description', 'special_col': 'special target'}]
        self.assertEqual(expected_json, target_info)
