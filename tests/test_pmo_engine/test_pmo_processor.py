...
class TestPMOProcessor(unittest.TestCase):
    ...
    def test_count_targets_per_library_sample(self):
        ...
        targets_per_sample_check_df = pd.DataFrame(
            targets_per_sample_check_data)
        pd.testing.assert_frame_equal(
            targets_per_sample_check_df, targets_per_sample_counts)

        targets_per_sample_counts_read_count_off1000 = PMOProcessor.count_targets_per_library_sample(
            pmo_data, 1000)
        ...
        targets_per_sample_read_count_off1000_check_df = pd.DataFrame(
            targets_per_sample_read_count_off1000_check_data)
        pd.testing.assert_frame_equal(
            targets_per_sample_counts_read_count_off1000, targets_per_sample_read_count_off1000_check_df)
        ...
        targets_per_sample_check_df_2 = pd.DataFrame(
            targets_per_sample_check_data_2)
        pd.testing.assert_frame_equal(
            targets_per_sample_check_df_2, targets_per_sample_check_df_2)

        targets_per_sample_counts_2_read_count_off200 = PMOProcessor.count_targets_per_library_sample(
            pmo_data_2, 200)
        ...
        targets_per_sample_check_df_2_read_count_off200 = pd.DataFrame(
            targets_per_sample_check_data_2_read_count_off200)
        pd.testing.assert_frame_equal(
            targets_per_sample_check_df_2_read_count_off200, targets_per_sample_counts_2_read_count_off200)

        with open(os.path.join(os.path.dirname(self.working_dir), "data/combined_pmo_example.json")) as f:
            pmo_data_combined = json.load(f)
        pmo_data_combined_targets_per_sample = PMOProcessor.count_targets_per_library_sample(
            pmo_data_combined)
        pmo_data_combined_targets_per_sample_check = pd.DataFrame([
            {"bioinformatics_run_id": 0,
                "library_sample_name": "8025874266", "target_number": 85},
            {"bioinformatics_run_id": 0,
                "library_sample_name": "8025874217", "target_number": 99},
            {"bioinformatics_run_id": 1,
                "library_sample_name": "XUC009", "target_number": 98},
            {"bioinformatics_run_id": 1,
                "library_sample_name": "5tbx", "target_number": 100}
        ])
        pd.testing.assert_frame_equal(
            pmo_data_combined_targets_per_sample_check, pmo_data_combined_targets_per_sample)
    ...
    def test_count_library_samples_per_target(self):
        ...
        targets_per_sample_counts_collapsed = PMOProcessor.count_library_samples_per_target(
            pmo_data, collapse_across_runs=True)
        ...
        pd.testing.assert_frame_equal(
            targets_per_sample_counts_collapsed_check, targets_per_sample_counts_collapsed)
    ...
    def test_count_specimen_per_meta_fields(self):
        ...
        specimen_meta_fields_counts_check = pd.DataFrame([
            {"field": "collection_country",
                "present_in_specimens_count": 4, "total_specimen_count": 4},
            {"field": "collection_date", "present_in_specimens_count": 4,
                "total_specimen_count": 4},
            {"field": "geo_admin3", "present_in_specimens_count": 4,
                "total_specimen_count": 4},
            {"field": "host_taxon_id", "present_in_specimens_count": 4,
                "total_specimen_count": 4},
            {"field": "lat_lon", "present_in_specimens_count": 2,
                "total_specimen_count": 4},
            {"field": "parasite_density_info",
                "present_in_specimens_count": 2, "total_specimen_count": 4},
            {"field": "project_id", "present_in_specimens_count": 4,
                "total_specimen_count": 4},
            {"field": "specimen_collect_device",
                "present_in_specimens_count": 4, "total_specimen_count": 4},
            {"field": "specimen_name", "present_in_specimens_count": 4,
                "total_specimen_count": 4},
            {"field": "specimen_store_loc",
                "present_in_specimens_count": 4, "total_specimen_count": 4},
            {"field": "specimen_taxon_id", "present_in_specimens_count": 4,
                "total_specimen_count": 4},
    ...
    def test_count_specimen_by_field_value(self):
        ...
        specimen_meta_subfields_counts = PMOProcessor.count_specimen_by_field_value(
            pmo_data, ["collection_country"])

        specimen_meta_subfields_counts_check = pd.DataFrame([
            {"collection_country": "India", "specimens_count": 1,
                "specimens_freq": 0.25, "total_specimen_count": 4},
            {"collection_country": "Mozambique", "specimens_count": 2,
                "specimens_freq": 0.50, "total_specimen_count": 4},
            {"collection_country": "Papua New Guinea", "specimens_count": 1,
                "specimens_freq": 0.25, "total_specimen_count": 4},
        ])
        pd.testing.assert_frame_equal(
    ...
    def test_extract_allele_counts_freq_from_pmo(self):
        ...
        self.assertEqual(hashlib.md5(df_string).hexdigest(),
                         "79fadd12ff7033c23810afa6cd46b4a5")
        ...
        allele_counts_collapsed_string = pd.util.hash_pandas_object(
            allele_counts_collapsed).values
        self.assertEqual(hashlib.md5(allele_counts_collapsed_string).hexdigest(
    ...
    def test_extract_targets_insert_bed_loc(self):
        ...
        all_target_inserts = PMOProcessor.extract_targets_insert_bed_loc(
            pmo_data, sort_output=True)
        output1_fnp = os.path.join(
            self.test_dir.name, "all_target_inserts_test1.bed")
        PMOProcessor.write_bed_locs(all_target_inserts, output1_fnp)
        self.assertEqual("b7e477fe327ad7ae85f78ddaa66c313c",
                         md5sum_of_fnp(output1_fnp))

        all_target_inserts_8_6_10 = PMOProcessor.extract_targets_insert_bed_loc(
            pmo_data, [8, 6, 10, 20], sort_output=False)
        ...
        self.assertEqual("9be1da64a4794489e08eca11c240d879",
                         md5sum_of_fnp(output2_fnp))

        all_target_inserts_8_6_10_sorted = PMOProcessor.extract_targets_insert_bed_loc(
            pmo_data, [8, 6, 10, 20], sort_output=True)
        output3_fnp = os.path.join(
            self.test_dir.name, "all_target_inserts_test2_8_6_10_sorted.bed")
        PMOProcessor.write_bed_locs(
            all_target_inserts_8_6_10_sorted, output3_fnp)
        self.assertEqual("1831cc6a6f9f2bc4036d1dfad90771a1",
                         md5sum_of_fnp(output3_fnp))
    ...
    def test_extract_panels_insert_bed_loc(self):
        ...
        output_fnp = os.path.join(
            self.test_dir.name, "all_panel_inserts_test1.bed")
        PMOProcessor.write_bed_locs(all_target_inserts, output_fnp)
        self.assertEqual("52b1f79a3a89f8265573fa54b5a7ce57",
                         md5sum_of_fnp(output_fnp))

    def test_extract_alleles_per_sample_table(self):
        with open(os.path.join(os.path.dirname(self.working_dir), "data/combined_pmo_example.json")) as f:
            pmo_data = json.load(f)
        allele_data = PMOProcessor.extract_alleles_per_sample_table(pmo_data).sort_values(
            by=['bioinformatics_run_name', 'library_sample_name', 'target_name', 'mhap_id'])
        output_fnp = os.path.join(
            self.test_dir.name, "extracted_alleles_per_sample_table_no_extra_args.csv")
        allele_data.to_csv(output_fnp, index=False)
        self.assertEqual("d1775ec03eb38743cd4dd92d0a832bff",
                         md5sum_of_fnp(output_fnp))
        ...
        output_fnp = os.path.join(
            self.test_dir.name, "extracted_alleles_per_sample_table_no_extra_args_with_seq_reads.csv")
        allele_data_with_seq_reads.to_csv(output_fnp, index=False)
        self.assertEqual("0e5da30c561c748fb2553f852db76607",
                         md5sum_of_fnp(output_fnp))
        ...
        output_fnp = os.path.join(
            self.test_dir.name, "extracted_alleles_per_sample_table_no_extra_args_with_seq_reads_panel_id_collection_country.csv")
        allele_data_with_seq_reads_panel_id_collection_country.to_csv(
            output_fnp, index=False)
        self.assertEqual("13aed17cbdff88f0a80c685c42d89cb8",
                         md5sum_of_fnp(output_fnp))
    ...
    def test_extract_from_pmo_with_read_filter(self):
        ...
        self.assertEqual("cde277533e44e64c3ef980106024275f",
                         md5sum_of_fnp(output_fnp))
    ...
    def test_filter_pmo_by_target_ids(self):
        with open(os.path.join(os.path.dirname(self.working_dir), "data/combined_pmo_example.json")) as f:
            pmo_data = json.load(f)
        pmo_data_select_targets = PMOProcessor.filter_pmo_by_target_ids(pmo_data, {
                                                                        1, 10, 11, 55})
        output_fnp = os.path.join(
            self.test_dir.name, "combined_pmo_target_ids_1_10_11_55.json")
        with open(output_fnp, "w") as f:
            json.dump(pmo_data_select_targets, f)
        self.assertEqual("7bb125130685a955d658fe5c03634024",
                         md5sum_of_fnp(output_fnp))
    ...
    def test_filter_pmo_by_target_names(self):
        ...
        pmo_data_select_targets = PMOProcessor.filter_pmo_by_target_names(
            pmo_data, {"t96", "t80", "t34", "t55"})
        output_fnp = os.path.join(
            self.test_dir.name, "combined_pmo_target_ids_t96_t80_t34_t55.json")
        with open(output_fnp, "w") as f:
            json.dump(pmo_data_select_targets, f)
        self.assertEqual("4d546b332b264cf58ed0d44a687728b9",
                         md5sum_of_fnp(output_fnp))
        # check pmo extracted against PMO schema
        checker = PMOChecker(self.pmo_jsonschema_data)
        checker.validate_pmo_json(pmo_data_select_targets)
    ...
    def test_filter_pmo_by_library_sample_ids(self):
        with open(os.path.join(os.path.dirname(self.working_dir), "data/combined_pmo_example.json")) as f:
            pmo_data = json.load(f)
        pmo_data_select_targets = PMOProcessor.filter_pmo_by_library_sample_ids(pmo_data, {
                                                                                1, 3})
        output_fnp = os.path.join(
            self.test_dir.name, "combined_pmo_example_ids_1_3.json")
        with open(output_fnp, "w") as f:
            json.dump(pmo_data_select_targets, f)
        self.assertEqual("96e9ddc3a3d582617d856b01dd3f5083",
                         md5sum_of_fnp(output_fnp))
        # check pmo extracted against PMO schema
        checker = PMOChecker(self.pmo_jsonschema_data)
        checker.validate_pmo_json(pmo_data_select_targets)
    ...
    def test_filter_pmo_by_library_sample_names(self):
        with open(os.path.join(os.path.dirname(self.working_dir), "data/combined_pmo_example.json")) as f:
            pmo_data = json.load(f)
        pmo_data_select_library_sample_names = PMOProcessor.filter_pmo_by_library_sample_names(
            pmo_data, {"8025874217", "XUC009"})
        output_fnp = os.path.join(
            self.test_dir.name, "combined_pmo_example_ids_8025874217_XUC009.json")
        with open(output_fnp, "w") as f:
            json.dump(pmo_data_select_library_sample_names, f)
        self.assertEqual("f7637b5f2b100ca316102c13283493b0",
                         md5sum_of_fnp(output_fnp))
        # check pmo extracted against PMO schema
        checker = PMOChecker(self.pmo_jsonschema_data)
    ...
    def test_filter_pmo_by_specimen_ids(self):
        with open(os.path.join(os.path.dirname(self.working_dir), "data/combined_pmo_example.json")) as f:
            pmo_data = json.load(f)
        pmo_data_select_targets = PMOProcessor.filter_pmo_by_specimen_ids(pmo_data, {
                                                                          0, 2})
        output_fnp = os.path.join(
            self.test_dir.name, "combined_pmo_specimen_ids_0_2.json")
        with open(output_fnp, "w") as f:
            json.dump(pmo_data_select_targets, f)
        self.assertEqual("ce94ed180bd6672d69371e3e0fcf0c4e",
                         md5sum_of_fnp(output_fnp))
        # check pmo extracted against PMO schema
        checker = PMOChecker(self.pmo_jsonschema_data)
        checker.validate_pmo_json(pmo_data_select_targets)
    ...
    def test_extract_from_pmo_samples_with_meta_groupings(self):
        with open(os.path.join(os.path.dirname(self.working_dir), "data/combined_pmo_example.json")) as f:
            pmo_data = json.load(f)
        pmo_data_select_meta, group_counts = PMOProcessor.extract_from_pmo_samples_with_meta_groupings(
            pmo_data, "collection_country=Mozambique")
        output_fnp = os.path.join(
            self.test_dir.name, "combined_pmo_collection_country_Mozambique.json")
        with open(output_fnp, "w") as f:
            json.dump(pmo_data_select_meta, f)
        self.assertEqual("b7c0c2c1c5fbfd9a023af3dfd6bf44c0",
                         md5sum_of_fnp(output_fnp))
        # check pmo extracted against PMO schema
        checker = PMOChecker(self.pmo_jsonschema_data)
    ...
    def test_get_sorted_bioinformatics_run_names(self):
        with open(os.path.join(os.path.dirname(self.working_dir), "data/combined_pmo_example.json")) as f:
            pmo_data_combined = json.load(f)
        names = PMOProcessor.get_sorted_bioinformatics_run_names(
            pmo_data_combined)
        self.assertEqual(['Mozambique2018-SeekDeep',
                         'PathWeaver-Heome1'], names)
