...
class TestPMOChecker(unittest.TestCase):
    def setUp(self):
        ...
        self.pmo_required_base_fields = ["library_sample_info",
                                         "specimen_info",
                                         "sequencing_info",
                                         "panel_info",
                                         "target_info",
    ...
    def test_pmo_checker_get_required_fields_for_pmo_class(self):
        self.assertEqual(self.checker.get_required_fields_for_pmo_class(
            "SpecimenInfo"), self.specimen_required_fields)
    ...
    def test_pmo_checker_validate_pmo_json_fail(self):
        with open(os.path.join(os.path.dirname(self.working_dir), "data/minimum_pmo_example_bad_format.json")) as f:
            pmo_data = json.load(f)
        self.assertRaises(
            ValidationError, self.checker.validate_pmo_json, pmo_data)
