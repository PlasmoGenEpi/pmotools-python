...
def parse_args_terra_amp_output_to_json():
    ...
    parser.add_argument('--gt_sheet', type=str, default="gt", required=False, help='The gt sheet to convert, if none provided will default to gt')
    parser.add_argument('--asv_table_sheet', type=str, default="asv_table", required=False, help='The asv_table sheet to convert, if none provided will default to asv_table')
    parser.add_argument('--asv_seqs_sheet', type=str, default="asv_seqs", required=False, help='The asv_seqs sheet to convert, if none provided will default to asv_seqs')
...
def terra_amp_output_to_json():
    ...
    asv_seqs  = pd.read_excel(args.file, sheet_name=args.asv_seqs_sheet, index_col="asv_id")
    asv_seq_table_look_up = dict()
    for index, row in asv_seqs.iterrows():
        asv_seq_table_look_up[index] = row["asv_seq"]


    representative_ref_seqs = {"representative_microhaplotype_id" : representative_microhaplotype_id,
                               "targets" : {}}
    asv_table  = pd.read_excel(args.file, sheet_name=args.asv_table_sheet, index_col="hapid")
    ...
    for index, row in asv_table.iterrows():
        asv_table_look_up.update({index : {"asv_id" :index,
                                           "CIGAR" : row["CIGAR"],
                                           "CIGAR_masked" : row["CIGAR_masked"],
                                           "Amplicon": row["Amplicon"]}}
                                 )
        asv_table_look_up_by_target_by_cigar[row["Amplicon"]][row["CIGAR_masked"]] = index
        if row["Amplicon"] not in representative_ref_seqs["targets"]:
            representative_ref_seqs["targets"][row["Amplicon"]] = {"target_id": row["Amplicon"],
                                                                   "seqs": {} }
        CIGAR_masked =row["CIGAR_masked"]
    ...
    gt_contents = pd.read_excel(args.file, sheet_name=args.gt_sheet, index_col="Sample_id")
    warnings = []

    for index, row in gt_contents.iterrows():
        sample_data = {"experiment_sample_id": index,
                       "target_results": {}}
            ...
            if not (isinstance(row[amplicon_id], float) and pd.isna(row[amplicon_id])):
                ...
                for current_hap in current_haps:
                    ...
                    if len(current_hap_split) != 2:
                        warnings.append("sample: " + str(index) + " target: " + amplicon_id + "failed to process " + current_hap + " expected two values separated by :")
                    else:
                        read_cnt = float(current_hap_split[1])
                        cigar = current_hap_split[0]
                        if cigar not in asv_table_look_up_by_target_by_cigar[amplicon_id]:
                            warnings.append("sample: " + str(
                                index) + " target: " + amplicon_id + "failed to find " + cigar + " in cigar look up")
                        else:
                            amplicon_data.append(
                                {"microhaplotype_id": asv_table_look_up_by_target_by_cigar[amplicon_id][cigar],
                                 "read_count" : read_cnt}
                            )
            if len(amplicon_data) > 0:
                sample_data["target_results"].update({
                    amplicon_id : {"target_id" : amplicon_id,
                                   "microhaplotypes" : amplicon_data}
                })
        microhaplotypes_detected["experiment_samples"].update(
            {index : sample_data}
        )
    if len(warnings) > 0:
        raise Exception("\n".join(warnings))
