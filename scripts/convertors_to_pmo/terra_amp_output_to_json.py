...
def parse_args_terra_amp_output_to_json():
    ...
    parser.add_argument('--gt_sheet', type=str, default="gt", required=False,
                        help='The gt sheet to convert, if none provided will default to gt')
    parser.add_argument('--asv_table_sheet', type=str, default="asv_table", required=False,
                        help='The asv_table sheet to convert, if none provided will default to asv_table')
    parser.add_argument('--asv_seqs_sheet', type=str, default="asv_seqs", required=False,
...
def terra_amp_output_to_json():
    ...
    asv_table_look_up_by_target_by_cigar = defaultdict(
            ...
            if not (isinstance(row[amplicon_id], float) and pd.isna(row[amplicon_id])):
                ...
                for current_hap in current_haps:
                    ...
                    if len(current_hap_split) != 2:
                        warnings.append("sample: " + str(index) + " target: " + amplicon_id +
                                        "failed to process " + current_hap + " expected two values separated by :")
                    else:
