...
def parse_args_extract_pmo_with_select_library_sample_names():
    ...
    parser.add_argument('--library_sample_names', type=str, required=True, help='Can either comma separated library_sample_names, or a plain text file where each line is a library_sample_name')
...
def extract_pmo_with_select_library_sample_names():
    ...
    all_library_sample_names = set(Utils.parse_delimited_input_or_file(args.library_sample_names))

    # read in pmo
    pmo = PMOReader.read_in_pmo(args.file)

    # extract
    pmo_out = PMOProcessor.filter_pmo_by_library_sample_names(pmo, all_library_sample_names)
