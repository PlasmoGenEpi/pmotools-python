...
def parse_args_extract_for_allele_table():
    ...
    parser.add_argument('--jsonschema',
                        default=os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "etc/portable_microhaplotype_object.schema.json"),
                        type=str, required=False, help='the jsonschema to check the PMO against')
    ...
    parser.add_argument('--library_sample_info_meta_fields', type=str, required=False, help='Meta Fields if any to include from the library sample table')
    ...
    parser.add_argument('--default_base_col_names', type=str, required=False, default="library_sample_name,target_name,mhap_id", help='default base column names, must be length 3')

    return parser.parse_args()

def extract_for_allele_table():
    args = parse_args_extract_for_allele_table()
    compressed_output = ("." not in args.output and args.file.endswith(".gz")) or args.output.endswith('.gz')

    output_delim, output_extension = Utils.process_delimiter_and_output_extension(args.delim, gzip=compressed_output)
    ...
    if args.library_sample_info_meta_fields is not None:
        args.library_sample_info_meta_fields = Utils.parse_delimited_input_or_file(args.library_sample_info_meta_fields, ",")
    ...
    allele_table = PMOProcessor.extract_alleles_per_sample_table(pmodata,
